    ...
    <div class="col-lg-9 col-md-12">
      <div class="table-responsive table-wrapper">
        <table class="table table-bordered table-hover excel-table">
          ...
          <tbody>
            <tr v-for="(intervencion, index) in intervenciones" :key="index" @click="openModal(intervencion.hora)"
...
export default {
  ...
  data() {
    return {
      ...
      selectedTime: '',
      selectedIntervencion: null, // Store the selected intervencion
    };
  },
  computed: {
    selectedMedicoName() {
  ...
  created() {
    this.fetchMedicos();
    this.intervenciones = this.generateTimeSlots(); // Initialize the table with all time slots
  },
  methods: {
    fetchMedicos() {
      this.loading = true;
    ...
    async openModal(hora) {
      ...
      const url = `/api/intervenciones/check?${params.toString()}`;
      ...
      try {
        ...
        if (response.ok && contentType && contentType.includes('application/json')) {
          const intervencion = await response.json();
          console.log('Intervención found:', intervencion); // Debug line
          if (intervencion && Object.keys(intervencion).length > 0) {
            this.selectedIntervencion = intervencion; // Set the selected intervencion
          } else {
            this.selectedIntervencion = null; // No matching intervencion found
          }
        } else {
          const responseText = await response.text();
          console.error('Expected JSON response but got:', contentType, responseText);
          this.selectedIntervencion = null; // Error in fetching intervencion
        }
      } catch (error) {
        console.error('Error checking intervención:', error);
        this.selectedIntervencion = null; // Error in fetching intervencion
      }

      this.$refs.intervencionModal.openModal(); // Open the modal
    },
    generateTimeSlots() {
      const slots = [];
    ...
    async fetchIntervenciones() {
      if (!this.selectedMedico || !this.selectedFecha) {
        this.intervenciones = this.generateTimeSlots();
        return;
      }

      // Use local date format
      const formattedDate = this.formatLocalDate(this.selectedFecha);
      const url = `/api/intervenciones?medico=${this.selectedMedico}&fecha=${formattedDate}`;

      try {
        const response = await fetch(url);
        if (response.ok) {
          const data = await response.json();
          console.log('Raw intervenciones data:', data);
          
          const slots = this.generateTimeSlots();
          if (Array.isArray(data)) {
            // Build intervencionesMap with proper time formatting
            const intervencionesMap = new Map();
            
            // Process each intervencion
            data.forEach(intervencion => {
              // Extract time from fecha field (assuming format "YYYY-MM-DD HH:mm:ss")
              const timeStr = intervencion.hora ? intervencion.hora.substring(0, 5) : null;
              if (timeStr) {
                intervencionesMap.set(timeStr, intervencion);
              }
            });

            // Map slots to intervenciones
            this.intervenciones = slots.map(slot => {
              const matchingIntervencion = intervencionesMap.get(slot.hora);
              
              if (matchingIntervencion) {
                const pacienteData = matchingIntervencion.paciente || {};
                ...
                return {
                  hora: slot.hora,
                  historia: matchingIntervencion.num_historia || '',
                  paciente: pacienteNombre,
                  tipo_intervencion: matchingIntervencion.tipoIntervencion ? matchingIntervencion.tipoIntervencion.tipo_intervencion : ''
                };
              }
              return {
                ...slot,
            ...
            this.intervenciones = slots.map(slot => ({ ...slot, tipo_intervencion: '' }));
          }
        }
      } catch (error) {
        console.error('Error:', error);
        this.intervenciones = this.generateTimeSlots().map(slot => ({ ...slot, tipo_intervencion: '' }));
      }
    }
  },
  watch: {
